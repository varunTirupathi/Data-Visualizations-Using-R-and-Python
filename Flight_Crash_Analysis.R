...
air_crash_df$LocationCountry <- gsub(".*,", "", air_crash_df$Location)

air_crash_df$LocationCountry <- str_trim(air_crash_df$LocationCountry, side = "both")
#Converting string back to factors
air_crash_df$LocationCountry <- sapply(air_crash_df$LocationCountry, as.factor)
...
air_crash_df$`Aboard Crew` <- sapply(air_crash_df$`Aboard Crew`, as.numeric)
...
years_graph <- years %>%
  ...
  ggtitle("Figure 7. Total number of crashes per year") +
...
Fatalities %>%
  ...
  scale_x_discrete(breaks = seq(from = 1908, to = 2019, by = 10)) +
  ggtitle("Figure 13.Percent of fatalities per year") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
...
Location_Crash %>%
  ...
  ggtitle("Figure 3.Top 10 Countries with Maximum Fatalities") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
pie3D(
  Location_Crash$total_fatalities,
  labels = Location_Crash$LocationCountry,
  main = "Figure 4. Top 10 Countries with Maximum Fatalities"
)
...
ggplot(operator[1:10,], aes(
  ...
  ggtitle("Figure 5. Top 10 Aircraft Operator causing Aircrash") +
  coord_flip() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
...
ggplot(type[1:10,], aes(
  ...
  ggtitle("Figure 6. Top 10 Aircraft Type causing Aircrash") +
  coord_flip() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
# end of Figure 8. code for finding the top 10 aircraft type causing aircrashes
#start of Figure 9. Code for finding highest/lowest total passengers and total fatalities count from the year 1970 to the 2019
view(head(total_fetalities_passengers))
...
fig <- plot_ly(x = total_fetalities_passengers$Year) %>%
  add_lines(
...
fig %>% layout(title = 'Figure 12. Passengers/Fatalities')
...
ggplot(crash_of_aeroflot, aes(y = Freq, x = Year, group = 1)) +
  ...
  ggtitle("Figure 11. Count of Accidents by year (Aeroflot)") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
...
ggplot(datamining[1:20,], aes(
  ...
  ggtitle("Figure 14.Common air crash words") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) + coord_flip()
...
ggplot(datamining_sentences[1:20,],
  ...
  xlab("Crash Phrases") + ylab("Count") +
  ggtitle("Figure 15. Common air crash phrases") +
  coord_flip() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
...
ggplot(df_time_freq,
  ...
  ggtitle("Figure 10. Air crashes per hour") +
  coord_flip() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
...
ggplot(day, aes(y = Freq, x = Var1, group = 1)) +
  ...
  ggtitle("Figure 8.Total number of crashes per day") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
view(day)
...
ggplot(crashes_day_df,
       aes(x = Day_Name, y = CRASHES_TOTAL_DAY, alpha = CRASHES_TOTAL_DAY)) +
  geom_bar(stat = "identity", fill = "maroon", width = 0.5) +
  xlab("Day") + ylab("Total crashes") +
  ggtitle("Figure 9. Crashes on each day of the week") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
