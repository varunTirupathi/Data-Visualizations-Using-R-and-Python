...
write.csv(
  air_delay_df ,
  "C:\\Varun\\Year II\\Big Data Project\\Project I\\airline delay analysis\\Sampled\\final_master_df.csv"
)
...
airline_codes_df <-
  read_csv(
    "C:\\Varun\\Year II\\Big Data Project\\Project I\\airline delay analysis\\Sampled\\Airline_Codes_Csv.csv"
  )
air_delay_df <-
  merge(air_delay_df,
        airline_codes_df,
...
data %>%
  ...
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) + theme(text = element_text(size = 15))
...
plot_daily_delay <- daily_delay_df %>%
  ...
  labs(title = "Figure 3. Total delay/Day",
       x = "Day",
       y = "Total delay/Mil") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) + geom_line(size = 2) + theme(text = element_text(size = 15))
plot_daily_delay
...
plot_yearly_delay <- yearly_delay_df %>%
  ...
  labs(title = "Total delay/Year",
       x = "Year",
       y = "Total delay/Mil") + theme(panel.grid = element_blank()) + geom_line(size = 2) +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) + theme(text = element_text(size = 15))
plot_yearly_delay
...
plot_usmap(
  ...
  labs(title = "Figure 5. Total delay in each State") + theme(text = element_text(size = 15))
...
plot_overall_delay_carrier <- delay_summed_df %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Figure 8. Total delay per carrier", x = "Carrier", y = "Total delay/Hour") +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) + theme(text = element_text(size = 12))
...
plot_distance_per_carrier <- delay_by_distance_df %>%
  ...
  labs(title = "Total distance travelled by carrier", x = "Carrier", y =
...
cancellations_2017_df %>%
  ...
  geom_smooth(size = 1) + theme(text = element_text(size = 15))
...
tom_cancelled_dest %>%
  ...
  geom_bar(aes(fill = CANCELLATION_REASON)) + theme(text = element_text(size = 15))  #, position = "dodge") + 
...
plot_categorzied_day_delay <- delay_day_in_rows_df %>%
  ...
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) + coord_flip() + theme(text = element_text(size = 15))
